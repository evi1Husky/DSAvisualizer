...
const AnimatedList = (() => {
    ...
    const setAnimation = (animationType, animationValue) => {
        switch (animationType) {
            case 'popUp':
                animations.popUp = [...animationValue];
                break;
            case 'popUpOptions':
                animations.popUpOptions = Object.assign({}, animationValue);
                break;
            case 'shrink':
                animations.shrink = [...animationValue];
                break;
            case 'shrinkOptions':
                animations.shrinkOptions = Object.assign({}, animationValue);
                break;
            case 'traverse':
                animations.traverse = [...animationValue];
                break;
            case 'traverseOptions':
                animations.traverseOptions = Object.assign({}, animationValue);
                break;
            default:
                throw new Error(`Unsupported animation type: ${animationType}`);
        }
    };
    function getNodeList(listContainerID) {
        const listContainer = document.querySelector(`#${listContainerID}`);
        if (!listContainer) {
            throw new Error('cannot find list container');
        }
        return Array.from((listContainer === null || listContainer === void 0 ? void 0 : listContainer.children) || []);
    }
    const getNode = (nodes, key, index) => {
        if (key === 'first') {
            return nodes[0];
        }
        else if (key === 'last') {
    ...
    const animateNode = (nodeToAnimate, animationType) => {
        if (!nodeToAnimate) {
    ...
    const calculateDelayForTraversal = (nodes, animationDuration, selectedNode) => {
        const node = nodes.find(node => node.getAttribute('data-value') === selectedNode);
    ...
    const traverseNodesAnimation = (nodes, animationDuration, selectedNode, animateLastTraversedNode) => {
    ...
    let listSize = 0;
    let listContainerID;
    const animate = (listMethod, selectedNode, animationType, itemToInsert, animateLastTraversedNode) => {
        const startAnimation = (animationType, selectedNode, nodeIndexToAnimate) => {
        ...
        const traversePopUp = () => {
            const traversingNodeDuration = animations.traverseOptions.duration;
            const nodes = getNodeList(listContainerID);
            const nodeIndexToAnimate = traverseNodesAnimation(nodes, traversingNodeDuration, selectedNode, animateLastTraversedNode);
            setTimeout(() => {
                listMethod(selectedNode, itemToInsert);
                listSize += 1;
                const interval = setInterval(() => {
                    const started = startAnimation('popUp', String(itemToInsert), nodeIndexToAnimate);
                    if (started) {
                        clearInterval(interval);
                    }
                }, 16.7);
