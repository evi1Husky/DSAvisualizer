const AnimatedList = (() => {
  ...
  const getNode = (nodes: Element[], key: string, index?: number): Element | null => {
    if (key === 'first') {
  ...
  const animate = (
    ...
    const traversePopUp = (): void => {
      ...
      const nodeIndexToAnimate = traverseNodesAnimation(
        nodes, traversingNodeDuration, selectedNode, animateLastTraversedNode
      )
      setTimeout(() => {
        ...
        const interval = setInterval(() => {
          const started =
            startAnimation('popUp', String(itemToInsert), nodeIndexToAnimate)
          if (started) {
            clearInterval(interval)
          }
        }, 16.7);
